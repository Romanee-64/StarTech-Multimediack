class Audio{
  Button btnchoose, btnplay, btnpause, btnstop, btnshuffle;
  String getFile = null;
  ...
  void makeButton(int x){
    ...
    btnshuffle = new Button(x, btnstop.y+btnstop.sizeY, loadImage("shuffle.png"));
  }
  ...
  void buttonPressed(String str){
    ...
    //shuffleボタンが押されたらランダムで選択
    if(btnshuffle.isPush()){
      int n = int(random(6)+1);
      String title = str + str(n) + ".mp3";
      choosetitle = title;
      getFile = dataPath("") + "/" + title;
      fileLoader();
      playingtitle = choosetitle;
    }
  }
